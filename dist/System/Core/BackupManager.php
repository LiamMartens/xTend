			...
			**/
			...
			private function needsBackup() {
				if($this->_app->getBackupInterval()!==false) {
					...
					$backups = $this->_app->getDirectoryHandler()->files($this->_app->getDirectoryHandler()->systemDirectory("Backups")); sort($backups);
					if(count($backups)>0) {
			...
			private function cleanBackups() {
				if($this->_app->getBackupLimit()!==false) {
					$backups = $this->_app->getDirectoryHandler()->files($this->_app->getDirectoryHandler()->systemDirectory("Backups"), true); sort($backups);
