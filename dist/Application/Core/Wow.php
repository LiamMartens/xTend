        ...
        */
        public function compileView($file, $layout_dir = false, $modules_dir = false) {
            ...
            if($compile_view) {
                ...
                $compiled_string="<?php namespace ".$this->_app->getNamespace()."; \$app=\\xTend\\Core\\getCurrentApp(__NAMESPACE__); \$controller=\$app->getControllerHandler()->getController(); \$controllers=\$app->getControllerHandler()->getControllersNames(); \$request=\$app->getRequestHandler()->getRequest(); ?>".$compiled_string;
                //write view output
